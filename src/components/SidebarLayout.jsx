...
const NAV_ITEMS = [
  ...
  { to: '/app/owner', label: 'Owner Portal', icon: Package, roles: ['ADMIN', 'CUSTOM_OFFICER', 'CONTAINER_OWNER', 'OWNER'] },
  ...
  { to: '/app/settings', label: 'Settings', icon: SettingsIcon, roles: ['ADMIN', 'CUSTOM_OFFICER', 'INSPECTOR', 'DRIVER', 'CONTAINER_OWNER', 'OWNER'] },
];
...
export function CgoLogo({ size = 96 }) {
  return (
    <svg viewBox="0 0 100 100" width={size} height={size} role="img" aria-label="CGO Secure Transit">
      ...
      <text x="50" y="24" textAnchor="middle" fontSize="8" fontWeight="700" fill="#0B3A5A" letterSpacing="1">
        CGO
      </text>
      ...
      <text x="50" y="80" textAnchor="middle" fontSize="7" fontWeight="700" fill="#0B3A5A" letterSpacing="0.5">
        CGO
      </text>
      <text x="50" y="88" textAnchor="middle" fontSize="4.2" fill="#0B3A5A" letterSpacing="1">
        SECURE TRANSIT
      </text>
    </svg>
  );
...
function SidebarLayout() {
  ...
  const [currentUser, setCurrentUser] = useState({ username: 'User', role: 'ADMIN' });
      ...
      <aside className="flex w-64 flex-shrink-0 flex-col bg-[#0B3A5A]">
        ...
        <div className="border-t border-white/10 p-3">
          <div className="mb-2 rounded-lg bg-white/5 px-3 py-2.5">
            ...
            <p className="text-xs text-slate-400 capitalize">{currentUser.role.replace(/_/g, ' ')}</p>
          </div>
          <button
            type="button"
