...
function AdminPanel() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      // Fetch staff, drivers, and owners (using Promise.allSettled so if one endpoint fails, others still load)
      ...
      const staffRes = results[0].status === 'fulfilled' ? results[0].value.data : [];
      const driversRes = results[1].status === 'fulfilled' ? results[1].value.data : [];
      const ownersRes = results[2].status === 'fulfilled' ? results[2].value.data : [];

      const staffData = Array.isArray(staffRes) ? staffRes.map(u => ({
        id: u.staffId || u.id,
        type: 'STAFF', 
      ...
      const driverData = Array.isArray(driversRes) ? driversRes.map(u => ({
        id: u.driverId || u.id,
        type: 'DRIVER',
        firstname: u.firstname,
        lastname: u.lastname,
      ...
      const ownerData = Array.isArray(ownersRes) ? ownersRes.map(u => ({
        id: u.ownerId || u.id,
        type: 'OWNER',
        firstname: u.firstname,
        lastname: u.lastname,
    ...
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        if (type === 'STAFF') {
          ...
          await api.delete(`/api/admin/users/owners/${id}`);
        } else {
    ...
    if (window.confirm('Are you sure you want to deactivate this account?')) {
      try {
        if (type === 'STAFF') {
          ...
          await api.put(`/api/admin/users/owners/${id}/deactivate`);
          alert('Owner account deactivated');
        }
        fetchUsers();
      } catch (err) {
              ...
              <form onSubmit={handleAddUser} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="OWNER">Container Owner</option>
                  </select>
                </div>
                
                {/* Driver select kaloth vehicle number */}
                ...
                {/* Container Owner select kaloth company name */}
                {formData.accountType === 'CONTAINER_OWNER' && (
                  <div>
                    <label className="block text-xs font-medium text-slate-700 mb-1">Company / Organization</label>
                    <input type="text" name="companyName" value={formData.companyName} onChange={handleInputChange} placeholder="e.g. Ceylon Logistics Ltd" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:border-[#0B3A5A]" />
                  </div>
                )}
                    ...
                    <tr key={`${user.type}-${user.id}`} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${
                          user.role === 'CONTAINER_OWNER' || user.type === 'OWNER'
                            ? 'bg-sky-50 text-sky-700 font-semibold'
                            : user.type === 'DRIVER'
                            ? 'bg-orange-50 text-orange-600'
                            : 'bg-blue-50 text-blue-600'
                        }`}>
                          {user.role === 'CONTAINER_OWNER' ? 'Container Owner' : user.role}
                        </span>
                      </td>
                      <td className="whitespace-nowrap px-4 py-3">
